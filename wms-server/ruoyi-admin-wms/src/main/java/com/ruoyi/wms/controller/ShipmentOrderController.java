...
public class ShipmentOrderController extends BaseController {
    ...
    /**
     * 出库（带AGV联动）：选阀门、生成出库任务、托盘出库调用AGV
     */
    @SaCheckPermission("wms:shipment:all")
    @Log(title = "出库单", businessType = BusinessType.UPDATE)
    @RepeatSubmit()
    @PostMapping("/shipmentWithAgv")
    public R<String> shipmentWithAgv(@RequestParam Long shipmentOrderId,
        ...
        String agvTaskId = shipmentOrderService.shipmentWithAgv(
            shipmentOrderId, valveNos, fromBinCode, toBinCode);
    ...
    public R<Void> agvCallback(@RequestParam String taskNo, @RequestParam Boolean success) {
        shipmentOrderService.onAgvTaskCompleted(taskNo, success);
        return R.ok();
    }
}
