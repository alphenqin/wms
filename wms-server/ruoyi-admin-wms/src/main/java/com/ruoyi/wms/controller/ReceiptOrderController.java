...
public class ReceiptOrderController extends BaseController {
    ...
    /**
     * 入库（带AGV联动）：托盘扫码、阀门绑定、库位分配、AGV任务生成
     */
    @SaCheckPermission("wms:receipt:all")
    @Log(title = "入库单", businessType = BusinessType.UPDATE)
    @RepeatSubmit()
    @PostMapping("/receiveWithAgv")
    public R<String> receiveWithAgv(@RequestParam Long receiptOrderId,
                                     @RequestParam String palletCode,
                                     @RequestParam List<String> valveNos,
                                     @RequestParam(required = false) String fromBinCode,
                                     @RequestParam(required = false) String toBinCode) {
        String agvTaskId = receiptOrderService.receiveWithAgv(
            receiptOrderId, palletCode, valveNos, fromBinCode, toBinCode);
    ...
    public R<Void> agvCallback(@RequestParam String taskNo, @RequestParam Boolean success) {
        receiptOrderService.onAgvTaskCompleted(taskNo, success);
        return R.ok();
    }
}
