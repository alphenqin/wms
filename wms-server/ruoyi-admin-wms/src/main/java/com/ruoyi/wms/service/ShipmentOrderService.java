...
public class ShipmentOrderService {
    ...
    public String shipmentWithAgv(Long shipmentOrderId, List<String> valveNos, 
                                  String fromBinCode, String toBinCode) {
        // 1. 验证出库单
        ShipmentOrderVo shipmentOrder = queryById(shipmentOrderId);
        ...
        if (palletCode == null) {
            throw new BaseException("未找到关联的托盘");
        }

        // 3. 更新阀门状态为已出库
        for (String valveNo : valveNos) {
            var valveVo = valveService.queryByValveNo(valveNo);
            valveService.updateStatus(valveVo.getId(), 3); // 3:已出库
        }
        ...
        if (fromBinCode != null && toBinCode != null) {
            try {
                agvTaskId = agvIntegrationService.dispatchOutboundTask(
                    palletCode, fromBinCode, toBinCode, shipmentOrder.getOrderNo(), shipmentOrderId);
            } catch (Exception e) {
                // AGV任务下发失败，记录日志但不影响出库流程
            }
        }
    ...
    public void onAgvTaskCompleted(String taskNo, boolean success) {
        ...
        if (success && agvTaskVo.getBizOrderId() != null) {
            // 更新出库单状态为已完成
            ShipmentOrderVo shipmentOrder = queryById(agvTaskVo.getBizOrderId());
